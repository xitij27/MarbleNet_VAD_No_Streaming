...
class DataLoadFolders:
    ...
    def _get_matching_file_pairs(
        ...
        if len(missing_audio_stems) > 0 or len(missing_rttm_stems) > 0:
            logging.warning("mismatch between audio and rttm files detected.")
            if len(missing_audio_stems) > 0:
                logging.warning("missing audio files at %s:", audio_paths[0].parent)
                logging.warning(missing_audio_paths)
            if len(missing_rttm_stems) > 0:
                logging.warning("missing rttm files at %s:", rttm_paths[0].parent)
                logging.warning(missing_rttm_paths)
